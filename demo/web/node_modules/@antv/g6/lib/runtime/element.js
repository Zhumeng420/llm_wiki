...
class ElementController {
    ...
    draw(context = { animation: true }) {
        ...
        return this.context.animation.animate(animation, silence
            ? {}
            : {
                before: () => this.emit(new event_1.GraphLifeCycleEvent(constants_1.GraphEvent.BEFORE_DRAW, { dataChanges, animation, render: type === 'render' }), context),
                ...
                after: () => this.emit(new event_1.GraphLifeCycleEvent(constants_1.GraphEvent.AFTER_DRAW, { dataChanges, animation, render: type === 'render' }), context),
            });
    }
    ...
    getFrontZIndex(id) {
        ...
        if (elementType === 'combo') {
            ...
            if (!(0, collapsibility_1.isCollapsed)(elementData)) {
                const ancestors = model.getAncestorsData(id, constants_1.COMBO_KEY).map(id_1.idOf);
                data.nodes = data.nodes.filter((element) => !ancestors.includes((0, id_1.idOf)(element)));
                data.combos = data.combos.filter((element) => !ancestors.includes((0, id_1.idOf)(element)));
