...
class Fisheye extends base_plugin_1.BasePlugin {
    ...
    prevMagnifiedStyleMap = new Map();
    prevOriginStyleMap = new Map();
    renderFocusElements = () => {
        if (!this.isLensOn)
            return;
        ...
        nodeData.forEach((datum) => {
            ...
            magnifiedStyleMap.set(nodeId, { ...(0, point_1.toPointObject)(newPoint), ...style });
            originStyleMap.set(nodeId, { ...(0, point_1.toPointObject)(position), ...originStyle });
        });
    ...
    updateStyle = (magnifiedStyleMap, originStyleMap) => {
    ...
    bindEvents() {
        ...
        if (scaleRBy === 'wheel' || scaleDBy === 'wheel') {
            ...
            this.graphDom?.addEventListener(constants_1.CommonEvent.WHEEL, wheelFunc, { passive: false });
        }
    }
    unbindEvents() {
        const { graph } = this.context;
        ...
        if (scaleRBy === 'wheel' || scaleDBy === 'wheel') {
            const wheelFunc = scaleRBy === 'wheel' ? this.scaleRByWheel : this.scaleDByWheel;
            this.graphDom?.removeEventListener(constants_1.CommonEvent.WHEEL, wheelFunc);
        }
    }
    update(options) {
        this.unbindEvents();
