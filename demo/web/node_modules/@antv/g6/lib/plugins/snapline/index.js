...
class Snapline extends base_plugin_1.BasePlugin {
    ...
    initSnapline = () => {
        ...
        if (!this.horizontalLine) {
            this.horizontalLine = canvas.appendChild(new g_1.Line({ style: { ...defaultLineStyle, ...this.options.horizontalLineStyle } }));
        }
        if (!this.verticalLine) {
            this.verticalLine = canvas.appendChild(new g_1.Line({ style: { ...defaultLineStyle, ...this.options.verticalLineStyle } }));
        }
    };
    getNodes() {
        ...
        const nodes = allNodes.filter((node) => {
            return (0, element_1.isVisible)(node) && this.context.viewport?.isInViewport(node.getRenderBounds());
        });
        if (!filter)
            return nodes;
    ...
    isHorizontalSticking = false;
    isVerticalSticking = false;
    enableStick = true;
    autoSnapToLine = async (nodeId, bbox, metadata) => {
    ...
    calcSnaplineMetadata = (target, nodeBBox) => {
