...
class EdgeFilterLens extends base_plugin_1.BasePlugin {
    ...
    bindEvents() {
        ...
        if (scaleRBy === 'wheel') {
            this.graphDom?.addEventListener(constants_1.CommonEvent.WHEEL, this.scaleRByWheel, { passive: false });
        }
    }
    unbindEvents() {
        const { graph } = this.context;
        ...
        if (scaleRBy === 'wheel') {
            this.graphDom?.removeEventListener(constants_1.CommonEvent.WHEEL, this.scaleRByWheel);
        }
    }
    update(options) {
        this.unbindEvents();
