const ORIGINAL_MAP = new WeakMap();
/**
 * <zh/> 设置图形实例的可见性
 *
 ...
 */
export function setVisibility(shape, value, inherited = false, filter) {
    if (value === undefined)
        return;
    const traverse = (current, scope = value) => {
        const walk = (val = scope) => current.childNodes.forEach((node) => traverse(node, val));
        ...
        if (!inherited && current === shape) {
            ...
            if (!ORIGINAL_MAP.has(current))
                ORIGINAL_MAP.set(current, current.style.visibility);
